...
class Client(object):
    ...
    def login(self):
        login = 'https://cursos.uo.edu.cu/login/index.php'
    ...
    def upload_file(self, file,user_id):
        try:
            ...
            post_file_url = 'https://cursos.uo.edu.cu/repository/repository_ajax.php?action=upload'
    ...
    def getFiles(self,user_id):
        try:
            ...
            post_file_url = 'https://cursos.uo.edu.cu/repository/draftfiles_ajax.php?action=list'
    ...
    def delete_files(self,user_id):
        try:
            ...
            post_delete = 'https://cursos.uo.edu.cu/repository/draftfiles_ajax.php?action=deleteselected'
    ...
    def rename_files(self,file):
        try:
            fileurl = 'https://cursos.uo.edu.cu/user/edit.php?id=18753&returnto=profile'
            resp=self.session.get(fileurl)
            soup = BeautifulSoup(resp.text, 'html.parser')
            sesskey = soup.find('input', attrs={'name':'sesskey'})['value']
            #_qf__user_files_form = soup.find('input', attrs={'name':'_qf__user_files_form'})['value']
            ...
            print(payload)
            post_delete = 'https://cursos.uo.edu.cu/repository/draftfiles_ajax.php?action=updatefile'
            resp=self.session.post(post_delete,data=payload)
            print(resp.text)
            payload = {
                    'returnurl': fileurl,
