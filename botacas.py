...
def upload_to_moodle_yutu(url,filename, id):
    ...
    if req.status_code == 200:
        ...
        msg = bot.send_message(id,'Descargando...'+ filename)
        ...
        if loged:
            ...
            for f in multiFile.files:
                ...
                bot.send_message(id,f'Parte subida con exito')                
            bot.send_message(id,f'Archivo subido con éxito\n\nLink:{data}')


        else:
            bot.send_message(id,'No se pudo iniciar sesion')
...
def upload_to_moodle(url, id):
    ...
    if req.status_code == 200:
        ...
        try:
            ...
            bot.send_message(id,f'No se pudo iniciar session con: {rand_user[0]}')
        ...
        if get_file_size(filename)>chunk_size:
            ...
            if loged:
                ...
                for f in multiFile.files:
                    ...
                    moodle.delete_files(rand_user[1])
                    bot.send_message(id,f'Parte subida con exito')                
                bot.send_message(id,f'Archivo subido con éxito\n\n*{filename}*\n\nUser: {rand_user[0]}',parse_mode='markdown')
                bot.send_message(id,f'Link:{data}')
...
def docu(message):    
        bot.send_message(message.from_user.id,f'Descargando...')
        try:
            print(message)
